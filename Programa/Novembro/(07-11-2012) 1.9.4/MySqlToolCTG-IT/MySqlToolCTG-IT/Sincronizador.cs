...
namespace MySqlToolCTG_IT
{
    public partial class Sincronizador : Form
    {
        ...
        {
            if (Verifica.VerificaSeTemUsuarios(Encriptografar.EncriptografarString(tb_loginOnline.Text), Encriptografar.EncriptografarString(tb_senhaOnline.Text)))
            {                
        ...
        public void VerificaRegistrosLocal()
        {
            ...
            if (CAaccess._dataTable.Rows.Count > 0)
            {
                button3.Enabled = true;
            }
            else
        ...
        {
            ProcessStartInfo sInfo = new ProcessStartInfo("http://toolctg.com.br/pags/cadastro.php");
            Process.Start(sInfo);
            this.Close();
        }
