...
namespace MySqlToolCTG_IT
{
    public partial class TelaPrincipal : Form
    {
        ...
        private string UltimaTabelaSelecionada = "";
        private bool cancelarDelete = false;
        ...
        public string _UltimaTabelaSelecionada
        {
            get { return UltimaTabelaSelecionada; }
            set { UltimaTabelaSelecionada = value; }
        }

        public bool _cancelarDelete
        {
            get { return cancelarDelete; }
            set { cancelarDelete = value; }
        }
        ...
        public void PreencherTreeViewBancos(int posNode = 0)
        {
            ...
            {
                //Conexões Strip
                tsl_conexaoAtiva.Text = "Nem uma conexão ativa";
                tsl_bancoAtivo.Text = "Nem um banco ativa";
                tsl_usuarioAtual.Text = "NaN";
            }
        }
        ...
        public void ModificadorVisualInicial()
        {
            ...
            System.Threading.Thread ThrVerificaConexoes = new System.Threading.Thread(VerificaConexoes);
            ThrVerificaConexoes.Start();
        }
        ...
        public void VerificaFeedBack()
        {
            if (_FeedBack == false)
            {
                try
                {
                    ...
                    ControladoresBD.Visualizador("Select * from tbl_feedback where usuario = '" + Program._LoginUsuario + "/" + usu_Windows.Name + "' and versao = '" + Program._VersaoPrograma + "';", 0);
            ...
            try
            {
                ...
                if (tv_bd.SelectedNode.ImageIndex == 0)
                {
                    ...
                    else
                    {
                        ...
                        _db = CAaccess._dataSet.Tables[0].Rows[0]["nome_banco"].ToString();
                        ConectionStringBruta(_db);
                        PreencherTreeViewBancos(tv_bd.SelectedNode.Index);
                        tsl_tabelaAtiva.Text = "Nem uma tabela ativa";
                    ...
                    {                        
                        if (_db == tv_bd.SelectedNode.Text)
                        {
                            return;
                        }
                        else
        ...
        public void ConectionStringBruta(string _nomeBD = "mysql")
        {
            ...
            //Conexões Strip
            tsl_conexaoAtiva.Text = _nomeConexao;
            tsl_bancoAtivo.Text = _nomeBD;
            tsl_usuarioAtual.Text = _usuario;
        }
        ...
        {
            if (_UltimoConexaoSelecionada !=  "")
            {
                novaTabela criarTabela = new novaTabela();
                criarTabela.ShowDialog();
                CarregaTables();
            }
            else
            {
        ...
        private void lv_tabelas_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                if (lv_tabelas.SelectedItems.Count > 0 || lv_tabelas.SelectedItems.Count > 1)
                {
                    tabControl_principal.TabPages.Remove(this.tab_tabelasPesq);
                    tabControl_principal.TabPages.Add(this.tab_tabelasPesq);
                    tabControl_principal.SelectTab(tab_tabelasPesq);
                    AtualizaDgDadosArmazem(lv_tabelas.SelectedItems[0].Text);
                    tsl_tabelaAtiva.Text = lv_tabelas.SelectedItems[0].Text;
                }
                else
                {
                    tsl_tabelaAtiva.Text = "";
                }
            }
            catch
            {
        ...
        public void AtualizaDgDadosArmazem(string Tabela)
        {
            ...
            ControladoresBD.Visualizador("Select * from " + Tabela, 1, "dg_dadosArmazenados");
            tab_tabelasPesq.Text = "Tabela: " + Tabela;
            ...
            ControladoresBD.Visualizador("Desc " + Tabela, 1, "data_dsc");
            dataGrid_desc.DataSource = ControladoresBD._dataSet;
            dataGrid_desc.DataMember = "data_dsc";

            _UltimaTabelaSelecionada = Tabela;
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    ControladoresBD.Modificador(rtb_cmds.Text, true);
                    AtualizaDgDadosArmazem(_UltimaTabelaSelecionada);
                }
            }
            catch
            {
        ...
        {
            if (MessageBox.Show("Você realmente deseja fazer Logoff?","Atenção!",MessageBoxButtons.YesNo,MessageBoxIcon.Question) == DialogResult.Yes)
            {
                Application.Restart();
            }
        }
        ...
        private void inserirNovosDadosToolStripMenuItem_Click(object sender, EventArgs e)
        {
            Program._UltimaTabelaSelecionada = _UltimaTabelaSelecionada;
        ...
        {
            System.Threading.Thread DeletarRegistrosSel = new System.Threading.Thread(DeletarRegistrosSelecionados);
            DeletarRegistrosSel.Start();
        }     
   
        public void DeletarRegistrosSelecionados()
        {
            try
            {
                ...
                if (MessageBox.Show("Registros selecionados: " + linhas + "\n\nVocê realmente deseja deletar o(s) registro(s) selecionado(s)?", "Atenção!", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
                {
                    ...
                    {
                        string _SQL = "DELETE FROM " + _UltimaTabelaSelecionada + " WHERE";
                        ...
                        ControladoresBD.Modificador(_SQL, true, "registro(s) deletado(s) com sucesso!");
                        AtualizaDgDadosArmazem(_UltimaTabelaSelecionada);
                    }
                    else
                    {
                        ...
                        for (int i = 0; i < dataGrid_dadosArmazenados.SelectedRows.Count; i++)
                        {
                            if (_cancelarDelete == false)
                            {
                                ...
                                _SQL = "DELETE FROM " + _UltimaTabelaSelecionada + " WHERE";
                        ...
                        MessageBox.Show("Registros apagados com sucesso!", "Atenção!", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                }
                else
                {
        ...
        private void atualizarDadosSelecionadosToolStripMenuItem_Click(object sender, EventArgs e)
        {
            //for (int x = 0; x < dataGrid_dadosArmazenados.SelectedRows.Count; x++)
            //{
            //    for (int y = 0; y < dataGrid_dadosArmazenados.ColumnCount; y++)
            //    {
            //        Program._dadosUp.Rows[x].Table.Rows.Add(dataGrid_dadosArmazenados.SelectedRows[x].ToString());
            //    }
            //}
            //dataGrid_dadosArmazenados.DataSource = Program._dadosUp;
        }
        ...
        {
            MessageBox.Show("Shift + Esc = Fecha a aba selecionada.", "HotKeys", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
        }

        private void lv_tabelas_KeyUp(object sender, KeyEventArgs e)
        {
            if (lv_tabelas.SelectedItems.Count > 0)
            {
                if (MessageBox.Show("Deseja realmente excluir esta tabela?", "Excluir Tabela", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) == DialogResult.Yes)
        ...
        private void lsb_ultimos_comandos_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                if (lsb_comandos_criacao.Items.Count > 0)
        ...
        {
            if (MessageBox.Show("Este recurso está em fase de testes, não nos responsabilizamos pela perda parcial ou total da informação aqui compartilhada.\n\nVocê deseja continuar mesmo assim?", "Atenção!", MessageBoxButtons.YesNo, MessageBoxIcon.Information) == DialogResult.Yes)
            {
                Sincronizador sincronizar = new Sincronizador();
